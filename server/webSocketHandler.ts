...
// ==== WebChucK Preload Complete Handler ====
    private handlePreloadComplete(sessionId: string, data: any) {
        ...
        this.logger.info(`Session ${sessionId} preload complete: ${data.status}`);

        // Store preload results in session debug info
        session.debugInfo.lastPreloadResult += data.message;
...
// ==== WebChucK Execute Code Error Handler ====
    private handleExecuteCodeError(sessionId: string, data: any) {
        ...
        this.logger.info(`Session ${sessionId} execution error updated to: ${data.status}`);
...
// ==== WebChucK Execute Code Error Handler ====
    private handlePreloadError(sessionId: string, data: any) {
        const session = sessionsManager.get(sessionId);
        if (session)
            session.debugInfo.lastPreloadResult += data.error;

        this.logger.info(`Session ${sessionId} preload error updated to: ${data.status}`);
        this.mcpServer.server.notification({
            method: 'error',
            params: {
                message: data.message,
