...
import {refreshSampleKeywords} from "./ui.js";
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Sample Keyword UI
    refreshSampleKeywords();
...
function setupEventListeners() {
    ...
    UI.DOMElements.codeLibraryBtn().addEventListener('click', handleCodeLibraryToggle);
    UI.DOMElements.codeEditorBtn().addEventListener('click', handleCodeLibraryToggle);
    ...
    UI.DOMElements.doUploadSamplesBtn().addEventListener('click', handleUploadSampleToggle);
    UI.DOMElements.doRecordSamplesBtn().addEventListener('click', handleRecordSampleToggle);
    UI.DOMElements.cancelUploadSamplesBtn().addEventListener('click', handleUploadSampleToggle);
    UI.DOMElements.cancelRecordSamplesBtn().addEventListener('click', handleRecordSampleToggle);
    ...
    UI.DOMElements.libraryList().addEventListener('dblclick', handleLoadFromLibrary);
...
async function handleConnectionStatusTitleClick(code) {
    // If the WebChucK instance and server is connected - allows expanded className toggled on parent
...
function handleCodeLibraryToggle() {
    UI.toggleCodeLibrary();
}

async function handleUploadSampleToggle() {
    const uploadSubsection = UI.DOMElements.uploadSampleSection();
    const searchSubsection = UI.DOMElements.searchSamplesSection();
    searchSubsection.classList.toggle('hidden');
    uploadSubsection.classList.toggle('hidden');
}
async function handleRecordSampleToggle() {
    const recordSubsection = UI.DOMElements.recordSampleSection();
    const searchSubsection = UI.DOMElements.searchSamplesSection();
    searchSubsection.classList.toggle('hidden');
    recordSubsection.classList.toggle('hidden');
}
...
async function handleSearchSamples() {
    ...
    // Empty will return and output none found.
    const files =
        !searchQuery
            ? []
            : await ServerConnection.fetchSamplesList(searchQuery);
    if (files) {
        console.log(files);
...
`;
        ...
        UI.filterSamplesSelector(searchQuery);
    }
}
